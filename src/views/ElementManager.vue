<template>
  ...
  <div class="flex justify-center items-center gap-10 p-2 m-5" role="group">
    <button @click="$router.push('/report')"
      class="p-2.5 text-regular font-medium bg-green-600 text-white rounded-md cursor-pointer shadow-md active:bg-green-800 transition-colors">
      <font-awesome-icon :icon="['fas', 'table-list']" /> Report</button>
    <button @click="$router.push('/events')" :disabled="store.events.length == 0"
      class="p-2.5 text-regular font-medium bg-green-600 text-white rounded-md cursor-pointer shadow-md active:bg-green-800 transition-colors disabled:text-gray-400 disabled:border-gray-400 disabled:bg-gray-200 disabled:cursor-not-allowed">
